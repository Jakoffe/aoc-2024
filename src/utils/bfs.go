...
type ShouldTravelFunc func(from Point, to Point) bool
type HandleVisitFunc func(p Point)
type HandleTravelBlockedFunc func(from Point, to Point, becauseAlreadyVisited bool)

var visited = map[Point]bool{}

func BFSMatrix[T comparable](matrix [][]T, startX int, startY int, shouldTravel ShouldTravelFunc, handleVisitFunc HandleVisitFunc, handleTravelBlockedFunc HandleTravelBlockedFunc) {
	rows := len(matrix)
	cols := len(matrix[0])
